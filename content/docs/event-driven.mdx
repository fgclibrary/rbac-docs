...
针对问题 1，仅需配置角色树的「选择命令」即可触发角色切换事件。但由于命令处理函数需要根据点击的节点类型进行不同处理，因此模块引入「组件」来分别呈现信息：

- `roleInfo`：展示角色信息。
- `roleTypeInfo`：展示角色分类信息。
- `roleGroupInfo`：展示角色权限组信息。

<Callout>
「选择命令」与「拖转命令」借助于「别名」机制，自成说明，详细逻辑可直接参考工程中的实现。
</Callout>

真正触发组件内部数据更新的关键，在于「操作单元格命令」中设置组件时传入的参数 `uniqueRoleKey`（角色唯一标识）。下面以 `roleInfo` 组件为例，说明事件驱动下的数据更新机制。

## 数据的局部更新

`roleInfo` 组件包含如下属性和事件：
...
需要注意的是，只有属性值发生变更时才会触发「属性值变更命令」进而加载数据。因此在设置默认值时，需要将 `administrator 的唯一标识` 与关键字 `reset` 交替设置，通过新旧值的切换来确保每次都能触发变更。该逻辑维护在 `roleMembership` 页面的属性值变更命令中。
